...
import { INTERNAL_LINK } from './ArticleLink';
...
export default class LinkCommand extends Command {
	...
	refresh() {
		...
		this.value = doc.selection.getAttribute( INTERNAL_LINK );

		for ( const manualDecorator of this.manualDecorators ) {
			manualDecorator.value = this._getDecoratorStateFromModel( manualDecorator.id );
		}

		this.isEnabled = model.schema.checkAttributeInSelection( doc.selection, INTERNAL_LINK );
	}
		...
		model.change( writer => {
			...
			if ( selection.isCollapsed ) {
				...
				for ( const range of ranges ) {
					writer.setAttribute( INTERNAL_LINK, href, range );
